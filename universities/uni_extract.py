...
with sync_playwright() as playwright:
    ...
    inner_side_elements = side_elements.query_selector(
        ".fltlft.filter-area.ctp_sidebar"
    )
    ...
    for name, code in city.items():
        ...
        try:
            ...
            substring = "rp_tuple_"
            div_elements = colz_div.query_selector_all(
                'div[id*="{0}"]'.format(substring)
            )
            ...
            def get_colleges_from_faculty_url(course_href, faculty_name, course_data):
                ...
                try:
                    ...
                    responsive_course_div = inner_course_div.query_selector(
                        "#rspnsv-tpl"
                    )
                    individual_divs = responsive_course_div.query_selector_all(
                        ".shadowCard.ctpCard.CLP"
                    )

                    for individual_div in individual_divs:
                        # name of course
                        colz_name = individual_div.query_selector(
                            "h3.instNamev2"
                        ).inner_text()
                        ...
                        # duration of course
                        year = (
                            individual_div.query_selector(".inlineLabel")
                            .inner_text()
                            .strip("\xa0()")
                            if individual_div.query_selector(".inlineLabel")
                            else ""
                        )

                        # for no of seats and total fee
                        more_div = individual_div.query_selector(
                            ".contSec.acpContainer"
                        )
                        ...
                        for row in rows:
                            label = (
                                row.query_selector(".blockLabel").inner_text().strip()
                            )
            ...
            for url in url_list:
                ...
                try:
                    ...
                    if len(li_elements) > 2:
                        ...
                        type_of_org = (
                            li_elements[1].inner_text().split("University")[0].strip()
                        )
                        data["type_of_org"] = type_of_org
                        # established_date
                        est_date = (
                            li_elements[2].inner_text().split("Estd. ")[1].strip()
                        )
                        ...
                        # type_of_org(public/private)
                        type_of_org = (
                            li_elements[0].inner_text().split("University")[0].strip()
                        )
                        data["type_of_org"] = type_of_org
                        # established_date
                        est_date = (
                            li_elements[1].inner_text().split("Estd. ")[1].strip()
                        )
                        data["est_date"] = est_date
                    else:
                        pass
                    ...
                    try:
                        body_section_div = collegedivpage.query_selector(
                            "#iulp_lhs_container"
                        )
                        inner_section = body_section_div.query_selector(
                            ".paper-card.spacingVariation"
                        )
                        about_div = inner_section.query_selector("._1003._58f5")
                        aaaa = about_div.query_selector(
                            ".wikiContents._021e.collapsed.ca53._5333.a78b"
                        )
                    ...
                    sections_main_div = coursepage.query_selector(
                        'section[data-tupletype="BAC"]'
                    )
                    section_container_div = sections_main_div.query_selector(
                        "._subcontainer"
                    )
                    section_individual_divs = section_container_div.query_selector_all(
                        ".shadowCard.ctpCard.BAC"
                    )
                    ...
                    for colz_div in section_individual_divs:
                        ...
                        print(course_href)
                        # courses
                        get_colleges_from_faculty_url(
                            course_href, faculty_data, data["course"]
                        )
                    ...
                    try:
                        ...
                        print(
                            "Error opening file(ignore this error, still creates the json)",
                            e,
                        )
                        results = []
                        with open(output_file, "w") as file:
            ...
            print(f"Error while finding college divs for city {name}: {e}")
            # Move to the next item in the dictionary
            continue
